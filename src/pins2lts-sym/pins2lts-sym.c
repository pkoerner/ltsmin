...
void inv_info_destroy(struct inv_info_s* info)
{
...
void inv_rel_destroy(struct inv_info_s* info) {
...
void inv_svar_destroy(struct inv_info_s* info) {
...
inv_info_prepare(ltsmin_expr_t e, ltsmin_parse_env_t env, int i)
{
    ...
    case PRED_GEQ: {
        ...
        struct inv_rel_s* rel = (struct inv_rel_s*) c + sizeof(struct inv_info_s);
        c->work = rel;
        
        rel->vec = (int*) rel + sizeof(struct inv_rel_s);
        GBgetInitialState(model, rel->vec);
        
        rel->len = len;
        rel->deps = (int*) rel->vec + sizeof(int[N]);
...
init_invariant_detection()
{
    ...
    for (int i = 0; i < num_inv; i++) {
        ...
        LTSminLogExpr(info, "optimized: ", inv_expr[i], inv_parse_env[i]);
