            ...
            switch (e->token) {
                ...
                case LTL_RELEASE: case LTL_UNTIL: {
                    ...
                    switch (e->token) {
                        ...
                        case LTL_NEQ: {
                            if (l->token == LTL_TRUE) { // true != a is !a, !a can be optimized
                                LTSminExprDestroy(l, 1);
                                LTSminExprDestroy(e, 0);
                                ltsmin_expr_t n = LTSminExpr(UNARY_OP, LTL_NOT, LTSminUnaryIdx(env, LTL_NAME(LTL_NOT)), r, 0);
                                return pred_tree_walk(n, env, lts_type);
                            }
                            if (r->token == LTL_TRUE) { // a != true is !a, !a can be optimized
                                LTSminExprDestroy(r, 1);
                                LTSminExprDestroy(e, 0);
                                ltsmin_expr_t n = LTSminExpr(UNARY_OP, LTL_NOT, LTSminUnaryIdx(env, LTL_NAME(LTL_NOT)), l, 0);
                                return pred_tree_walk(n, env, lts_type);
                    ...
                    if (LTSminExprEq(l, r)) {
                        switch (e->token) {
                            ...
                            case LTL_NEQ: { // a != a is false
                                ltsmin_expr_t n = LTSminExpr(CONSTANT, LTL_FALSE, LTSminConstantIdx(env, LTL_NAME(LTL_FALSE)), 0, 0);
                                pre_decorate(e, n);
                                init_decoration(n, env, lts_type);
                                n->annotation->chunk_type = type_check_get_type(lts_type, LTSMIN_TYPE_BOOL, e, env);
                                LTSminExprDestroy(e, 1);
            ...
            switch (e->token) {
                ...
                case CTL_UNTIL: {
                    ...
                    switch (e->token) {
                        ...
                        case CTL_NEQ: {
                            if (l->token == CTL_TRUE) { // true != a is !a, !a can be optimized
                                LTSminExprDestroy(l, 1);
                                LTSminExprDestroy(e, 0);
                                ltsmin_expr_t n = LTSminExpr(UNARY_OP, CTL_NOT, LTSminUnaryIdx(env, CTL_NAME(CTL_NOT)), r, 0);
                                return pred_tree_walk(n, env, lts_type);
                            }
                            if (r->token == CTL_TRUE) { // a != true is !a, !a can be optimized
                                LTSminExprDestroy(r, 1);
                                LTSminExprDestroy(e, 0);
                                ltsmin_expr_t n = LTSminExpr(UNARY_OP, CTL_NOT, LTSminUnaryIdx(env, CTL_NAME(CTL_NOT)), l, 0);
                                return pred_tree_walk(n, env, lts_type);
                    ...
                    if (LTSminExprEq(l, r)) {
                        switch (e->token) {
                            ...
                            case CTL_NEQ: { // a != a is false
                                ltsmin_expr_t n = LTSminExpr(CONSTANT, CTL_FALSE, LTSminConstantIdx(env, CTL_NAME(CTL_FALSE)), 0, 0);
                                pre_decorate(e, n);
                                init_decoration(n, env, lts_type);
                                n->annotation->chunk_type = type_check_get_type(lts_type, LTSMIN_TYPE_BOOL, e, env);
                                LTSminExprDestroy(e, 1);
